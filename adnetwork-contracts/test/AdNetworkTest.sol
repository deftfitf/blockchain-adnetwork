...
contract AdNetworkTest {
    ...
    function testCreateAdInventory() public {
        ...
        uint256 inventoryId = inventoryOwner.createAdInventory(name, uri, floorPrice);
    ...
    function testCreateAd() public {
        ...
        uint256 adId = adOwner.createAd{value: 0.01 ether}(inventoryId, hash, start, end);
...
contract AdOwner {
    ...
    function createAd(uint256 _inventoryId, bytes32 _hash, uint32 _start, uint32 _end) external payable returns (uint256) {
        return adNetwork.createAd{value: msg.value}(_inventoryId, _hash, _start, _end);
    }
...
contract InventoryOwner {
    ...
    function createAdInventory(string memory name, string memory uri, uint256 floorPrice) external returns (uint256) {
        return adNetwork.createAdInventory(name, uri, floorPrice);
    }
